...
class Alpaca {
  ...
  async mapStocks(params) {
    ...
    let newStocks = await newAlphaStocksToGet.reduce((chain, curr) => chain.then(async (prev) => {
      //get broker stock
      let brokerStock = await this.alp.getAsset(this._getStockSymbol({alpha_stock: curr}));
      //error if stock is not tradable
      if(!brokerStock.tradable) throw new Error('Security is not tradable.');
      //filter broker data
      let filteredData = {
      ...
      //set broker stock
      prev[brokerStock.symbol] = filteredData;
      if(curr.security === 'cryptocurrency') prev[_.replace(brokerStock.symbol, '/', '')] = filteredData;
      //return
      return prev;
    }), Promise.resolve({}));
