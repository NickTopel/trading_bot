...
class Bot {
  ...
  async calculateNewOrders() {
    ...
    while(remainingOrders.length > 0 && math.evaluate('a >= b', {a: remainingBuyingPower, b: this.Broker.minTotal})) {
      //pop remaining order
      let order = remainingOrders.pop();
      
      //calculate new order, push to current group
      let newOrderTotal = ((math.evaluate('a <= b', {a: order.total, b: currentGroupBuyingPower})) ? order.total : currentGroupBuyingPower);
      ...
      //reduce remainingBuyingPower and currentGroupBuyingPower by the new order total
      ...
      //if no remaining orders or buying power, push currentGroup to groupedOrders
      if(remainingOrders.length <= 0 || math.evaluate('a < b', {a: currentGroupBuyingPower, b: this.Broker.minTotal})) {
