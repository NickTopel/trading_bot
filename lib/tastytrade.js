...
class TastyTrade {
  ...
  async getInitialBuyingPower() {
    ...
    //return
    return accountBalances['equity-buying-power'];
  }
  ...
  async newOrder(params) {
    ...
    let fullAmount = math.fix(params.amount, brokerStock.precision_amount).toString();
    ...
    let orders = await Promise.all([wholeShares, fractionalShares].map(async (amount) => {
      ...
      if(params.type !== 'close' && math.evaluate('a < b', {a: total, b: this.minTotal})) return undefined;
      ...
      let order = await this._request({
        ...
        query: {
          'order-type': 'Market',
          'time-in-force': ((brokerStock.security === 'stock') ? 'Day' : 'IOC'),
          'legs': [
            {
              'instrument-type': ((brokerStock.security === 'stock') ? 'Equity' : 'Cryptocurrency'),
              'action': ((params.action === 'buy') ? 'Buy' : 'Sell')+' to '+((['buy_long', 'sell_short'].includes(params.type)) ? 'Open' : 'Close'),
              'quantity': amount,
              'symbol': brokerStock.id
            }
          ]
        }
      });
